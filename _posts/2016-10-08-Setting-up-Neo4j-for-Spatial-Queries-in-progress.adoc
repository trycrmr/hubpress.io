...
. Two of the procedures neo4j-spatial provides (as of October 2016) are `importShapefileToLayer(layerName :: STRING?, uri :: STRING?)` and `CALL spatial.addLayer("countryBorders","WKB","geometry")`. It takes two arguments: Whatever you would like to name the layer and a URI of where the Shapefiles are located. Validate these procedures exists then:
